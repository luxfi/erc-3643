...
/**
 * @title CreateX Factory Smart Contract (Minimal Version)
 * @notice Factory smart contract for CREATE3-based contract creations.
 * @dev This is a minimal version containing only the methods used by TREXFactory and tests.
 */
contract CreateX {
    ...
    /**
     * @dev Deploys and initialises a new contract via employing the `CREATE3` pattern (i.e. without an initcode
     * factor) and using the salt value `salt`, the creation bytecode `initCode`, the initialisation code `data`,
     * the struct for the `payable` amounts `values`, the refund address `refundAddress`, and `msg.value` as inputs.
     * In order to save deployment costs, we do not sanity check the `initCode` length. Note that if
     * `values.constructorAmount` is non-zero, `initCode` must have a `payable` constructor. This implementation
     * is based on Solmate:
    ...
    /**
     * @dev Returns the address where a contract will be stored if deployed via this contract using CREATE3.
     * @param salt The 32-byte random value used to create the proxy contract address.
     * @return computedAddress The 20-byte address where a contract will be stored.
     */
